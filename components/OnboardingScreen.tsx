          ...
          {isElectron && !codeRepoPath && (
            <div className="mb-4 p-3 bg-amber-50 rounded-lg border border-amber-200">
              ...
              <p className="text-xs text-amber-600">
                工作区文件和 Git 同步需要指定代码仓库根目录。点击下方"配置代码仓库路径"进行设置。
              </p>
            </div>
          )}
            ...
            <div className="mb-4">
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {workspaceFiles.map((filename) => (
                  <button
                    key={filename}
                    onClick={() => openSpecificWorkspace(filename)}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 bg-gray-50 rounded-lg hover:bg-blue-50 hover:text-blue-700 transition-colors border border-gray-100"
                  >
                    <svg
                      width="14"
                      height="14"
            ...
            <div className="mb-4 space-y-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  包含 .git 目录的项目根目录，工作区文件将保存在此目录的 taskData/ 下
                </p>
              </div>
              <div>
                <label className="text-xs text-gray-500 font-medium">
          ...
          <div className="flex flex-col gap-3">
            {isElectron && (
              <button
                ...
                className="w-full px-4 py-2.5 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
              >
                <svg
                  width="16"
                  height="16"
